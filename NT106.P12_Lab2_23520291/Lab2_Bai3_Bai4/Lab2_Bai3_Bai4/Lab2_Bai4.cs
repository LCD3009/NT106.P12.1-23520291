...
using static Lab2_Bai3_Bai4.Lab2_Bai4;
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                if (File.Exists(filePath))
                {
                    ...
                    if (fileInfo.Length > 0)
                    {
                        using (FileStream fsRead = new FileStream(filePath, FileMode.Open))
                        {
                            ...
                            try
                            {
                                existingStudents = (List<Student>)formatter.Deserialize(fsRead);
                            }
                            catch (Exception ex)
                            {
                ...
                if (nonDuplicateStudents.Count > 0)
                {
                    ...
                    using (FileStream fs = new FileStream(filePath, FileMode.Create))
                    {
                        ...
                        try
                        {
                            formatter.Serialize(fs, existingStudents);
                            listStudents.Clear(); // Xóa danh sách sinh viên
                            studentsListBox.Clear(); // Xóa hiển thị trong TextBox
        ...
        private void read_button_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                using (FileStream fs = new FileStream(filePath, FileMode.Open))
                {
                    ...
                    try
                    {
                        listStudents = (List<Student>)formatter.Deserialize(fs);
        ...
        private void calulatorAverage_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                using (FileStream fs = new FileStream(outputFilePath, FileMode.Create))
                {
                    ...
                    try
                    {
                        formatter.Serialize(fs, listStudents);
